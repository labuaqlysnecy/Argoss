...
```bash
git clone https://github.com/labuaqlysnecy/Argoss.git
cd Argoss
```
...
docker run -d \
  ...
  -v $(pwd)/logs:/app/logs \
  -v $(pwd)/data:/app/data \
  ghcr.io/labuaqlysnecy/argoss:2.0.0
```
...
1. Скачай APK из [последнего релиза](https://github.com/labuaqlysnecy/Argoss/releases/latest)
   или из [Actions → Android APK](https://github.com/labuaqlysnecy/Argoss/actions)
